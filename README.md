...
- [ ] Explain the differences between `client-side routing` and `server-side routing`.
      server-side routing is when clicking on a link changes a lot of things happen. For example, when click on a link the whole page refreshes. A new GET request has been made to the server, the server responds with a new document.

  Client side routing is when only elements on the page will change. The whole document will not refresh. When a user clicks on a link the route is handled by the JS on the page.

- [ ] What does HTTP stand for?
      HyperText transer protocol

- [ ] What does CRUD stand for?
      create read update delete which is the foundation of webpages.
-

[ ] Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers.
The POST method is used to create, Get is used to read, PUT is used for updating, and DELETE is used for deleting.
